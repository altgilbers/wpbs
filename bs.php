...
2018-09-19 - updated to allow for the selection of multiple sites
...
function bs_process()
{
	...
	// this should be an array, since the actual parameter name is "site_id[]"
	$sites=$_POST['site_id'];
	...
	$users = preg_split( "/[,;\r\n\s]/", $_POST['users'],-1,PREG_SPLIT_NO_EMPTY );
        ...
        $output.="Number of sites provided: ".count($sites)."\n";
...
function bs_init(){  ?>
   ...
   <form method="post" action="<?php echo network_admin_url( 'edit.php?action=bs_process' ) ?>">
      ...
      <br/>
      <select name="site_id[]" id="sites_list" multiple>
         <?php
